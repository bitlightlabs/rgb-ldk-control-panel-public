...
export default function CopyText(props: { text: string, className?: string }) {
  ...
  return (
    <Button
      className={
        cn("w-4 h-4 px-0 text-secondary-foreground hover:[&_svg]:text-foreground", className)
      }
      variant="ghost"
      onClick={copy}
...
export function CopyTextInline(
  ...
  return (
    ...
    >
      ...
      <Button
        className={
          cn("w-4 h-4 px-0 text-secondary-foreground hover:bg-transparent hover:[&_svg]:text-foreground", buttonClassName)
        }
        variant="ghost"
        onClick={copy}
        onKeyDown={(e) => e.stopPropagation()}
