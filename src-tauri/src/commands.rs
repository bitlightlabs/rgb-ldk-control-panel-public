...
async fn import_rgb_issuer_raw(
    ...
    let mut url =
        base.join("api/v1/rgb/issuers/import")
...
pub async fn bootstrap_local_environment(
    ...
    if contexts_file.exists() {
        ...
        stage_logs.push(format!(
            "Removed old contexts file: {}.",
            contexts_file.display()
        ));
    }
    ...
    if let Some(legacy_dirs) =
        directories::ProjectDirs::from("org", "rgbldk", "rgb-ldk-control-panel")
    {
    ...
    let (compose_template, compose_template_source) =
        if let Ok(v) = std::env::var("RGB_LDK_DOCKER_COMPOSE_FILE") {
            ...
            if p.exists() {
                ...
                (
                    content,
                    format!("env RGB_LDK_DOCKER_COMPOSE_FILE ({})", p.display()),
                )
            } else {
                return Err(CommandError::ExternalCommandFailed {
    ...
    stage_logs.push(format!(
        "Using compose template source: {compose_template_source}."
    ));
    ...
    stage_logs.push(
        "Reset previous compose containers/volumes for a clean local environment.".to_string(),
    );
  ...
  "contexts": [
    ...
    for _ in 0..60 {
        ...
        if !alex_unlocked
            && rgbldkd_http::control_unlock(&state.http, &alex)
                .await
                .is_ok()
        {
            if let Ok(st) = rgbldkd_http::control_status(&state.http, &alex).await {
        ...
        if !bob_unlocked
            && rgbldkd_http::control_unlock(&state.http, &bob)
                .await
                .is_ok()
        {
            if let Ok(st) = rgbldkd_http::control_status(&state.http, &bob).await {
